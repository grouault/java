...
public class TodosDwrImpl extends AbstractCommonDwr implements TodosDwr {
    ...
    public RemoteTodoList[] getCurrentTodoLists() {
        ...
        Collection<RemoteTodoList> remoteTodoLists = 
            new TreeSet<RemoteTodoList>();
        ...
        for (TodoList todoList : todoLists) {
            ...
            remoteTodoList.setDescription(todoList.getName() 
                    + " (" + completed + "/"
        ...
        return (RemoteTodoList[]) 
            remoteTodoLists.toArray(new RemoteTodoList[remoteTodoLists.size()]);
    }
    ...
    /**
     * @see tudu.web.dwr.TodosDwr#renderTodos(java.lang.String)
     */
    public String renderTodos(String listId) {
        log.debug("Render AJAX table");
        HttpServletRequest request = ExecutionContext.get()
        ...
        if (listId != null && !listId.equals("")) {
            try {
                ...
                request.setAttribute("todos", new TreeSet<Todo>(todos));
                int nbCompleted = 0;
                for (Todo todo : todos) {
                    if (todo.isCompleted()) {
                        nbCompleted++;
    ...
    public String reopenTodo(String todoId) {
        ...
        try {
            ...
            return renderTodos(todo.getTodoList().getListId());
        } catch (PermissionDeniedException pde) {
            return renderPermissionException();
        }
    }
