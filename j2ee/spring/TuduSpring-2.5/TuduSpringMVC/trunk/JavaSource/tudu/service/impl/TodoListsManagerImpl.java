 ...
 */
public class TodoListsManagerImpl implements TodoListsManager, ApplicationListener {
    ...
    public TodoList findTodoList(String listId) {
         ...
         if (!user.getTodoLists().contains(todoList)) {
             if (log.isInfoEnabled()) {
                 log.info("Permission denied when finding Todo List ID '" + listId
    ...
    public void updateTodoList(final TodoList todoList) {
        if (log.isDebugEnabled()) {
            log.debug("Update the Todo List with id " + todoList.getListId());
        }
    ...
    public void deleteTodoList(final String listId) {
        if (log.isDebugEnabled()) {
            log.debug("Delete the Todo List with id " + listId);
        }
        User user = userManager.getCurrentUser();
    ...
    /**
     * @see tudu.service.TodoListsManager#addTodoListUser(java.lang.String, java.lang.String)
     */
    public void addTodoListUser(String listId, String login) {
        TodoList todoList = this.findTodoList(listId);
        tudu.domain.model.User targetUser = userManager.findUser(login);
        todoList.getUsers().add(targetUser);
        targetUser.getTodoLists().add(todoList);
    }

    /**
     * @see tudu.service.TodoListsManager#deleteTodoListUser(java.lang.String, java.lang.String)
     */
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof ContextRefreshedEvent) {
            log.warn("Spring context is starting up : " + event.toString());
        }
    }
    ...
    public Document backupTodoList(TodoList todoList) {
        ...
        todoListElement.addContent(new Element("rss").addContent(String.valueOf(todoList
                .isRssAllowed())));
        
        Element usersElement = new Element("users");
        for (User user : todoList.getUsers()) {
            Element userElement = new Element("user");
            userElement.addContent(user.getLogin());
            usersElement.addContent(userElement);
        }
        todoListElement.addContent(usersElement);
    ...
    /**
     * @see tudu.service.TodoListsManager#restoreTodoList(java.lang.String, java.lang.String, java.io.InputStream)
     */
    public void restoreTodoList(String restoreChoice, String listId, InputStream todoListContent) 
    ...
    private void importTodosFromXml(TodoList todoList, Element rootElement) {
        ...
        for (Object todoObject : todos) {
            ...
            Todo todo = new Todo();
            Date creationDate = new Date(Long.parseLong(todoElement.getChildText("creationDate")));
            todo.setCreationDate(creationDate);
            todo.setDescription(todoElement.getChildText("description"));
            todo.setPriority(Integer.valueOf(todoElement.getChildText("priority")));
            todo.setCompleted(Boolean.parseBoolean(todoElement.getChildText("completed")));
            String completionDate = todoElement.getChildText("completionDate");
            if (completionDate != null) {
                todo.setCompletionDate(new Date(Long.parseLong(completionDate)));
            }
