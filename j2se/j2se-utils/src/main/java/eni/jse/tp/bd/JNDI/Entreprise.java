...
public class Entreprise {
	...
	public void etablirConnexion() {
		// try {
			...
			// MySqlDataSource => non présent dans le ClassPath
			/*
	...
	public void ajouterEmployes(List<Employe> listEmployes) {
		...
		try {
			if (!listEmployes.isEmpty()) {
				...
				// on peut r�cup�rer le r�sultat dans un tableau d'entier.
...
//			employ�3
			...
			// liste des employ�s dont le nom est rouault
			List<Employe> listEmployeRouault = entrepriseP.listeEmployeFamille("ROUAULT");
			System.out.println("Liste des employ�s dont le nom est 'ROUAULT':");
			if(!listEmployeRouault.isEmpty()){	
				...
				System.out.println("Aucun employ� porte ce nom.");
			}
